...
export default function GoldcarClient({ role }: Props) {
  ...
  async function handleAgregar() {
    ...
    try {
      await axios.put(api.habilitarGoldcar(cuenta.trim(), placa.trim(), "1"));
      await fetchUnidades();
    } catch {
  ...
  async function handleQuitar(accountID: string, deviceID: string) {
    ...
    try {
      await axios.put(api.habilitarGoldcar(accountID, deviceID, "0"));
      await fetchUnidades();
    } catch {
