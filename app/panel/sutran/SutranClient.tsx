...
export default function SutranClient({ role, actor }: Props) {
  ...
  async function handleAgregar() {
    ...
    try {
      await axios.put(api.habilitarSutran(cuenta.trim(), placa.trim(), "1", actor, motivo));
      await fetchUnidades();
    } catch {
  ...
  async function handleQuitar(accountID: string, deviceID: string) {
    ...
    try {
      await axios.put(api.habilitarSutran(accountID, deviceID, "0", actor, motivoQuitar));
      await fetchUnidades();
    } catch {
